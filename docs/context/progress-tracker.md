...
Phases 0–4, 4.5 (HUD + art), 4.6 (juice G1–G5), 6 A–D (walls, ranged enemy, smart chaser, **12-level run + tutorial**), **6.5 (Difficulty Levels)** and **Spec 022 (screen redesign + Title + Level Title)** complete on main, plus menus/audio/death screen and items Specs 011–012. Next: fix the audit findings below, then Phase 5 (ship).

## Current Spec

**Specs 016 + 024 (Phase 6 D) — merged to main** via PR #9 (`feature/run-structure-tutorials`), 2026-07-28. Smoke 65/65 PASS on main.

> **Merge trap worth remembering:** PR #8 (`feature/levels`) merged the *docs only* —
> specs 016/024, roadmap, tracker, 4 files. The implementation commit (`FEAT Phases`)
> was pushed to that branch *after* the merge, so it never reached main. It was re-cut
> onto `feature/run-structure-tutorials` and merged as PR #9. If a spec reads "done"
> but the code is missing, check whether the merge commit predates the implementation.

- **12 levels, 3 acts of 4**, all data: `LevelResource` per floor + `RunManager` + one shared `rooms/level.tscn`. Act I blue room/brown walls (chasers), Act II purple/teal (shooters), Act III red/green (mixed). Mazes generated by `WallPatterns` (`PEN`, `CROSS`, `TIC_TAC_TOE`, `RING`) into the WallTiles layer, with hard rules: never block a door approach, never seal a spawn.
- **Enemy cycles by tint (Rafael's call):** the two sprites repeat per act, recoloured and buffed — `EnemyStats.tint` + variants (Act I as authored / Act II steel-blue, +1 hit / Act III violet, +2 hits, faster). Damage darkening now starts from the variant tint.
- **Tutorial (024):** two parchment beats on floor 1 (controls incl. movement keys; then doors/items), pausing play, closed with SPACE, once per run; the tutorial enemy is penned so the player can practise safely.
- Two fixes found by screenshotting: Act III's red room palette was **missing from the tileset** (art existed, no tiles referenced it), and `RoomTiles` needed a `repaint()` because a child's `_ready()` runs before its parent's.
- Flow: New Game → difficulty → intro cutscene → level 1 → … → level 12 → win screen. `room_01/02` kept as smoke fixtures.
- **Smoke suite was dead on arrival and is now fixed** (see Technical Debt): a parse
  error meant it never loaded, and once loaded the tutorial's `paused = true` froze
  the world under 20 timed checks. Both fixed on main; 65/65 PASS.
- Open: Spec 019 (floor-scaled difficulty) now looks redundant — team call; enemy counts are a first pass.
...
3. **Level 12 sits at exactly zero spawn headroom.** `SPAWN_CELLS` offers 10 cells;
   ...
   spare). Adding one enemy or one wall cell silently drops an enemy, because
   `Level._spawn_enemies()` skips with `if index < cells.size()` and never warns.
   Worth a `push_warning` on shortfall regardless of whether counts change.
4. **Four independent owners of `get_tree().paused`**, none aware of the others:
   `TutorialOverlay`, `ItemAcquired`, `DeathScreen`, `Pause`. Each sets `false`
   unconditionally on close, so if two ever overlap the first to close unpauses the
   game while the other is still on screen. A tiny pause *counter* (or a single
   `Juice`-style owner) removes the whole class of bug.
5. **`ItemAcquired` leaks a timer per pickup.** `show_and_continue()` runs
   `get_tree().create_timer(AUTO_ADVANCE_SECONDS).timeout.connect(_continue)` on every
   call and never disconnects. `SceneTreeTimer`s survive scene changes, so a stale
   6 s timer from floor N can fire into floor N+1's screen and advance it early.
6. **`rooms/level.gd` reaches through untyped `set()`/`call()`** for `variant_row`,
   `repaint`, the door textures and `stats`. Against godot-standards' static-typing
   rule, and a rename fails silently at runtime instead of at parse time.
7. **Three scripts past the 250-line soft limit:** `enemy.gd` 341, `player.gd` 287,
   `magic_item.gd` 280. `enemy.gd` is the real candidate — chase, kite, lunge FSM and
   shooting in one file.
...
- **Smoke suite silently dead for a whole spec (found + fixed 2026-07-28, main):**
  two defects meant Specs 016/024 shipped with zero automated verification.
  (a) `tests/smoke_test.gd:480` reused `cell` as a `for` variable while `cell` was
  already declared at line 199 — GDScript scopes loop variables to the whole
  function, so the script failed to *parse* and the suite never loaded; a green
  console was really a suite that never ran. (b) With that fixed, 20 timed checks
  failed: `TutorialOverlay` is an autoload that sets `get_tree().paused = true` on
  `EventBus.level_entered`, `RunManager` defaults to level 1 which carries both
  beats, and the fixture rooms announce themselves like any real floor — so the
  world froze while the test's `SceneTreeTimer`s (which ignore pause) kept counting.
  Detached in the harness alongside the existing hitstop opt-out. 65/65 PASS.
  **Lesson:** a suite that cannot load looks exactly like a suite that passes unless
  you read its output — assert on the check count, not just the absence of FAIL.
