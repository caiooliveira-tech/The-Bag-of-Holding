...
- 2026-07-26 — **Room tiles integrated via TileMapLayer**: `rooms/room_tileset.tres` (32px atlas from the sheet) + `rooms/room_tiles.gd` paints floor/border at runtime (`variant_row` 0 = blue/room 1, 1 = pink/room 2; corner tile mirrored per quadrant; door-gap cells get floor). Door rebuilt as two 32px sprite leaves with closed/open textures per palette (graybox fallback kept). Collision unchanged (StaticBody2D). Graybox ColorRects removed from both rooms. Verified via `tests/screenshot_rooms.tscn`; smoke 21/21 PASS. Design template: cols 8–11 (closed door/open door/corner/floor), row 1/2 palettes.
